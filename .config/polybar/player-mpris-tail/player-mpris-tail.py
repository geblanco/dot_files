...
class PlayerStatus:
    ...
    def _on_metadata(self, player, e):
        # should be okay..., vlc does not set this (at least in custom tracks)
        if 'xesam:artist' in e.keys() and 'xesam:title' in e.keys():
            self._artist = e['xesam:artist'][0]
            self._title = e['xesam:title']
            self._print_song()
        elif 'xesam:url' in e.keys():
            url = urllib.parse.unquote(e['xesam:url']).split('/')
            self._artist = url[0]
            self._title = url[len(url) -1]
            self._print_song()
